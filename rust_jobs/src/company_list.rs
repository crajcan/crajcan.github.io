...
pub(crate) fn company_list(CompanyListProps{ companies, on_click }: &CompanyListProps) -> Html {
    ...
    companies.iter().map(|company| {
        ...
        let open_source = company.open_source_github_org.is_some();
        ...
        html! {
            ...
            <tr class="company" key={company.id} onclick={on_company_select}>
                ...
                <td class="company-meta">
                  if company.open_source_github_org.is_some() {
                  <div class={ format!("company-open-source{}", if company.startup {""} else { " no-startup" }) }>
                    <a class="company-open-source-link" href={company.open_source_github_org.clone().unwrap_or_default()}>
